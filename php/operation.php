...
function createData(){
    ...
    if ($bookname && $bookpublisher && $bookprice) {
        ...
        TextNode("error", "Please fill in all fields");
    }
}

function textboxValue($value){
    $textbox = mysqli_real_escape_string($GLOBALS['con'], trim($_POST[$value]));
    return empty($textbox) ? false : $textbox;
}

function TextNode($classname, $msg){
    echo "<h6 class='$classname'>$msg</h6>";
}
...
function UpdateData(){
    ...
    if ($bookid && $bookname && $bookpublisher && $bookprice) {
        ...
        TextNode("error", "Please fill out all fields before updating");
    }
}
...
function deleteRecord(){
    ...
    if (mysqli_query($GLOBALS['con'], $sql)) {
        ...
        TextNode("error", "Failed to Delete Record");
    }
}
...
function deleteAll(){
    ...
    if (mysqli_query($GLOBALS['con'], $sql)) {
        ...
        TextNode("error", "Failed to delete all records");
    }
}
